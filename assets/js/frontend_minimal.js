...
(function () {
  ...
  odoo.define = function () {
    ...
    var name =
      typeof args[0] === "string" ? args.shift() : "__odoo_job" + jobUID++;
  ...
  odoo.log = function () {
    ...
    if (jobs.length) {
      ...
      var unloaded = Object.keys(debugJobs)
        .map(function (key) {
  ...
  odoo.ready = async function (serviceName) {
    ...
    await Promise.all(
      jobs.filter((job) => match(job.name)).map((job) => job.promise)
    );
  ...
  odoo.runtimeImport = function (moduleName) {
    if (!(moduleName in services)) {
      throw new Error(
        `Service "${moduleName} is not defined or isn't finished loading."`
      );
    }
...
odoo.define("@web/legacy/js/core/menu", async function (require) {
  ...
  __exports.initAutoMoreMenu = initAutoMoreMenu;
  async function initAutoMoreMenu(el, options) {
    if (!el) {
      return;
    }
    ...
    const [breakpoint = "md"] = navbar
      ...
      : [];
    const isNoHamburgerMenu =
      !!navbar && navbar.classList.contains("navbar-expand");
    ...
    el.addEventListener("dom:autoMoreMenu:destroy", destroy, {
      once: true,
    });
    function _restore() {
      if (!extraItemsToggle) {
    ...
    function computeFloatOuterWidthWithMargins(
      el,
      mLeft,
      mRight,
      considerAutoMargins
  ...
  __exports.destroyAutoMoreMenu = destroyAutoMoreMenu;
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (header) {
      ...
      const excludedImages = [
        ...header.querySelectorAll(excludedImagesSelector),
      ];
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!window.frameElement) {
      ...
      window.frameElement.dispatchEvent(
        new CustomEvent("OdooFrameContentLoaded")
      );
    }
  });
  return __exports;
